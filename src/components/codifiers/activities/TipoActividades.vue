            ...
            <v-flex>
                <v-card outlined elevation="20">
                    ...
                    >
                        <template v-slot:top>
                            <v-toolbar flat>
                                <v-toolbar-title class="hidden-sm-and-down">Lista de tipos de actividades</v-toolbar-title>
                                <v-spacer></v-spacer>
                                <v-text-field  v-model="search" append-icon="mdi-magnify" label="Buscar"
                                              single-line hide-details></v-text-field>
                                <v-spacer></v-spacer>
                                <v-btn color="orange"  :fab="classButtons" :small="classButtons" @click="loadDataTable" dark v-resize="onResizes">
                                    <v-icon>mdi-refresh</v-icon>
                                    <span v-if="!classButtons">
                                         Refrescar
                                    </span>
                                </v-btn>
                                <v-btn color="primary" :fab="classButtons" :small="classButtons" class="ml-2" @click="openDialogNew=true" v-resize="onResizes">
                ...
                <v-dialog v-model="openDialogNew" transition="fade" persistent title="Nuevo tipo de actividad" max-width="520" >
                       <v-card>
                           <v-card-title>Nuevo tipo de actividad</v-card-title>
                           <v-divider></v-divider>
                           <v-container>
                               <v-form lazy-validation v-model="isFormValid" ref="formNew" @submit.prevent="handleNewTypeActivity">
                                   <v-layout row class="ma-1">
                                       <v-flex xs12>
                                           <v-text-field label="Nombre" v-model="typeActivity.nombre" :rules="rules.nombreRules" counter maxlength="50" ></v-text-field>
                                       </v-flex>
                                   </v-layout>
                                   <v-layout row class="ma-1">
                                       <v-flex xs12>
                                           <v-text-field label="Código" v-model="typeActivity.codigo" :rules="rules.codigoRules" counter maxlength="2"></v-text-field>
                                       </v-flex>
                                   </v-layout>
                                   <v-layout row class="ma-1">
                                       <v-flex xs12>
                                           <v-textarea auto-grow label="Descripción" v-model="typeActivity.descripcion" :rules="rules.descripcionRules" counter maxlength="250"></v-textarea>
                                       </v-flex>
                                   </v-layout>
                                   <v-layout row class="ma-1 text-right">
                                       <v-flex>
                                           <v-btn class="mr-1" color="success" type="submit" :loading="loading" :disabled="!isFormValid||loading">
                                               <span slot="loader" class="custom-loader">
                                                   <v-icon>mdi-refresh</v-icon>
                                               </span>
                                               Aceptar</v-btn>
                                           <v-btn class="ml-1" color="error" @click="handleCancelarNewDialog">Cancelar</v-btn>
                                       </v-flex>
                                   </v-layout>
                               </v-form>
                           </v-container>
                ...
                <v-dialog v-model="openDeleteDialog" persistent max-width="520">
                    <v-card>
                        <v-card-title v-if="typeActivityToDelete!==null||typeActivityToDelete.id>0">Eliminar actividad</v-card-title>
                        <v-divider></v-divider>
                        <v-container>
                        <h3 class="font-weight-light">¿Desea eliminar el tipo de actividad con código {{typeActivity.codigo}}?</h3>
                        <v-layout row class="ma-1 text-center">
                            <v-flex xs12>
                                <v-btn color="success" class="mr-1" @click="handleDeleteActivity" :loading="loading" :disabled="loading">
                                    <span slot="loader" class="custom-loader">
                                        <v-icon>mdi-refresh</v-icon>
                                    </span>
                                    Si</v-btn>
                ...
                <v-dialog v-model="openDialogEdit" transition="fade" persistent max-width="520" >
                    <v-card>
                        <v-card-title v-if="typeActivity!==null||typeActivity.id>0">Editar tipo de actividad con código {{typeActivity.codigo}}</v-card-title>
                        <v-divider></v-divider>
                        <v-container>
                            <v-form lazy-validation v-model="isFormValid" ref="formEdit" @submit.prevent="handleEditTypeActivity">
                                <v-layout row class="ma-1">
                                    <v-flex xs12>
                                        <v-text-field label="Nombre" v-model="typeActivity.nombre" :rules="rules.nombreRules" counter maxlength="50" ></v-text-field>
                                    </v-flex>
                                ...
                                <v-layout row class="ma-1 text-right">
                                    <v-flex>
                                        <v-btn class="mr-1" color="success" type="submit" :loading="loading" :disabled="!isFormValid||loading">
                                            ...
                                            Aceptar</v-btn>
                                        <v-btn class="ml-1" color="error" @click="handleCancelarEditDialog">Cancelar</v-btn>
                                    </v-flex>
                                </v-layout>
                            </v-form>
                        </v-container>
