...
<script>
    ...
    import {
        URL_DELETE_UNIDAD_MEDIDA, URL_EDIT_UNIDAD_MEDIDA,
        URL_GET_ALL_UNIDAD_MEDIDA,
        URL_SAVE_UNIDAD_MEDIDA
    ...
    export default {
        ...
        data() {
            return {
                ...
                loading: false,
                headers: [
                    {text: "Código", value: "codigoUnidadMedida", align: 'center'},
        ...
        methods: {
            ...
            editItem(item) {
                this.metricUnit.id = item.id;
                this.metricUnit.codigoUnidadMedida = item.codigoUnidadMedida;
                this.metricUnit.nombre = item.nombre;
                this.metricUnit.descripcion = item.descripcion;
                this.openDialogEdit = true;
                console.log(item)
            },
            dialogOpenDelete(item) {
                this.metricUnitToDelete = item;
            ...
            handleNewMetricsUnit() {
                if (this.$refs.formNew.validate()) {
                    this.loading = true;
                    const token = localStorage.getItem("token")
                    axios.post(URL_SAVE_UNIDAD_MEDIDA, this.metricUnit, {
                        headers: {
                            "Authorization": "Bearer " + token,
                            "cache-control": "no-cache",
                        }
            ...
            handleDeleteMetricUnit() {
                this.loading = true;
                const token = localStorage.getItem("token");
                axios.delete(URL_DELETE_UNIDAD_MEDIDA + this.metricUnitToDelete.id, {
                    headers: {
                        "Authorization": "Bearer " + token,
                        "cache-control": "no-cache",
                    }
            ...
            handleEditMetricsUnit() {
                if (this.$refs.formEdit.validate) {
                    this.loading = true;
                    const token = localStorage.getItem("token");
                    axios.put(URL_EDIT_UNIDAD_MEDIDA, this.metricUnit, {
                        headers: {
                            "Authorization": "Bearer " + token,
                            "cache-control": "no-cache",
                        }
