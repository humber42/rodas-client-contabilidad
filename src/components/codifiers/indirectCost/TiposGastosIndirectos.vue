            ...
            <v-layout row wrap>
                <v-flex>
                    <v-card outlined elevation="20">
                        <v-card-title class="hidden-md-and-up">Lista de tipos de gastos indirectos</v-card-title>
                        <v-skeleton-loader v-if="tableLoading" class="mx-auto" type="table"/>
                        <v-data-table v-else :headers="headers" :items="tiposGastosIndirectos" :items-per-page="5"
                                      class="elevation-1" :search="search"
                        >
                            <template v-slot:top>
                                <v-toolbar flat>
                                    <v-toolbar-title class="hidden-sm-and-down">Lista de tipos de gastos indirectos
                                    </v-toolbar-title>
                                    <v-spacer></v-spacer>
                            ...
                            <template v-slot:item.actions="{item}">
                                <v-tooltip bottom>
                                    ...
                                    <span>Editar tipo de gasto indirecto</span>
                                </v-tooltip>
                                <v-tooltip bottom>
                                    <template v-slot:activator="{on,attrs}">
                                    ...
                                    <span>Eliminar tipo de gasto indirecto</span>
                                </v-tooltip>
                            </template>
                        </v-data-table>
                    </v-card>
                    <v-dialog v-model="openDialogNew" transition="fade" persistent title="Nuevo tipo de gasto indirecto"
                              max-width="520">
                        <v-card>
                            <v-card-title>Nuevo tipo de gasto indirecto</v-card-title>
                            <v-divider></v-divider>
                            <v-container>
                                <v-form lazy-validation v-model="isFormValid" ref="formNew"
                                    ...
                                    <v-layout row class="ma-1">
                                        <v-flex xs12>
                                            <v-text-field label="Nombre" v-model="typeGastoIndirecto.nombre"
                                    ...
                                    <v-layout row class="ma-1">
                                        <v-flex xs12>
                                            <v-textarea auto-grow label="Descripción" v-model="typeGastoIndirecto.descripcion"
                    ...
                    <v-dialog v-model="openDeleteDialog" persistent max-width="520">
                        <v-card>
                            ...
                            <v-divider></v-divider>
                            <v-container>
                                <h3 class="font-weight-light">¿Desea eliminar el tipo de gasto indirecto con nombre
                    ...
                    <v-dialog v-model="openDialogEdit" transition="fade" persistent max-width="520">
                        <v-card>
                            <v-card-title v-if="typeGastoIndirecto!==null||typeGastoIndirecto.id>0">Editar tipo de gasto directo con nombre {{typeGastoIndirecto.nombre}}
                            </v-card-title>
                            <v-divider></v-divider>
                            <v-container>
...
<script>
    ...
    import {
        URL_DELETE_TIPOS_GASTOS_INDIRECTOS, URL_EDIT_TIPOS_GASTOS_INDIRECTOS,
        URL_GET_ALL_TIPOS_GASTOS_INDIRECTOS,
        URL_SAVE_TIPOS_GASTOS_INDIRECTOS
    ...
    export default {
        ...
        methods:{
            editItem(item){
                this.typeGastoIndirecto.id = item.id;
                this.typeGastoIndirecto.nombre=item.nombre;
                this.typeGastoIndirecto.descripcion=item.descripcion;
            ...
            loadDataTable(){
                ...
                axios.get(URL_GET_ALL_TIPOS_GASTOS_INDIRECTOS, {
                    ...
                    this.tiposGastosIndirectos = data;
                }).catch(err => {
                    console.log(err);
            ...
            handleNewTipoGasto(){
                if (this.$refs.formNew.validate()) {
                    this.loading = true;
                    const token = localStorage.getItem("token")
                    axios.post(URL_SAVE_TIPOS_GASTOS_INDIRECTOS, this.typeGastoIndirecto, {
                        headers: {
                            "Authorization": "Bearer " + token,
                            "cache-control": "no-cache",
                        }
                        ...
                        this.loading = false;
                        console.log(data)
                        this.tiposGastosIndirectos.push(data);
            ...
            handleDeleteTipoGasto() {
                this.loading = true;
                const token = localStorage.getItem("token");
                axios.delete( URL_DELETE_TIPOS_GASTOS_INDIRECTOS+ this.typeGastoToDelete.id, {
                    headers: {
                        "Authorization": "Bearer " + token,
                        "cache-control": "no-cache",
                    }
            ...
            handleEditTipoGasto() {
                if (this.$refs.formEdit.validate()) {
                    this.loading = true;
                    const token = localStorage.getItem("token");
                    axios.put(URL_EDIT_TIPOS_GASTOS_INDIRECTOS, this.typeGastoIndirecto, {
                        headers: {
                            "Authorization": "Bearer " + token,
                            "cache-control": "no-cache",
                        }
